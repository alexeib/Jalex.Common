...
namespace Jalex.Infrastructure.ReflectedTypeDescriptor
{
    internal class ReflectedTypeDescriptor<T> : IReflectedTypeDescriptor<T>
    {
        ...
        public ReflectedTypeDescriptor()
        {
            ...
            HasClusteredIndices = getHasClusteredIndices(Properties);
        ...
        private static bool getHasClusteredIndices(PropertyInfo[] classProps)
        {
            var hasClusteredIndices = (from prop in classProps
                                       let indexedAttribute = (IndexedAttribute)prop.GetCustomAttributes(true).FirstOrDefault(a => a is IndexedAttribute)
                                       where indexedAttribute != null && indexedAttribute.IsClustered
                                       select indexedAttribute).Any();
            return hasClusteredIndices;
        }
